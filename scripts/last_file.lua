...
--
--    local playlist_pos_file = io.open("~/.config/gnome-mpv/playlist_position.txt", "r")
...
function on_file_loaded(event)
    ...
    if playlist_count ~= 1 then
        local playlist_stored_pos = read_pos(".config/gnome-mpv/playlist_position.txt")
...
function write_playlist_pos(event)
    ...
    if playlist_count ~= 1 then
        local playlist_pos_output = io.open(".config/gnome-mpv/playlist_position.txt", "w")
